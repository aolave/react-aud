...
export class TableCustom extends PureComponent<props, TableCustomState> {
	...
	getAmountCount = (totalAmount: number = 0, countAll: number = 0) => {
    ...
    */
	handleCheckBox = (event: any) => {
		...
		if (checked === true) {
			...
			this.getAmountCount(sumAmount, SumCount);
		} else {
			sumAmount = totalAmount - moreAccount;
			SumCount = countAll - 1;
			this.setState({ totalAmount: sumAmount, countAll: SumCount });
		...
		if (custom === 1) {
			return data.column.map((h: any, i: any) => {
				if (h === 'isChecked')
					return (
						<th key={i}>
							<input type='checkbox' className='all-control-check' />
						</th>
					);
					...
					<td>
						<span className='ui red message'>
							...
							<Popup trigger={<Icon circular name='heart' />}>
								<Popup.Content>
									Hello. This is a regular pop-up which does not allow for lots of content. You cannot
									fit a lot of words here as the paragraphs will be pretty narrow.
								</Popup.Content>
							</Popup>
						</span>
					</td>
